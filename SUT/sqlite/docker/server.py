...
log(f"Starting sqlite (version: {sqlite3.sqlite_version})")
...
def openConn():
    ...
    conns[connID] = sqlite3.connect(f"{DB_DIR}/sqlite.db")
    cursors[connID] = conns[connID].cursor()
    cursors[connID].execute("BEGIN;")
...
def sql(payload: dict):
    ...
    cursors[connID].execute(query) # never do this, kids!
...
def fetchall(payload: dict):
    ...
    return {"status": "success", "result": cursors[connID].fetchall()}
